...
class Blinds(Hass):
    ...
    DEFAULT_CONFIG = {
        ...
        "delays": {
            ...
            "shadow_to_horizontal_delay": 600,
            "horizontal_to_neutral_delay": 900,
            "dawn_to_horizontal_delay": 60,
            "dawn_horizontal_to_neutral_delay": 900
        },
    ...
    def validate_config(self):
        ...
        if not self.params.get('entities'):
            ...
            if self.params['entities'].get('window_sensor'):
    ...
    def create_internal_entities(self):
        ...
        if not self.entity_exists(self.name_blinds_locked):
            ...
            self.set_state(entity_id=self.name_blinds_locked, state=STATE_OFF, attributes={"friendly_name": name, "icon": "mdi:lock"})
            self.log(f"Creating entity {self.name_blinds_locked} in HASS")
        ...
        if not self.entity_exists(self.name_blinds_locked_external):
            ...
            self.set_state(entity_id=self.name_blinds_locked_external, state=STATE_OFF, attributes={"friendly_name": name, "icon": "mdi:timer-lock"})
            self.log(f"Creating entity {self.name_blinds_locked_external} in HASS")
        ...
        if not self.entity_exists(self.name_manipulation_active):
            ...
            self.set_state(entity_id=self.name_manipulation_active, state=STATE_OFF, attributes={"friendly_name": name, "icon": "mdi:arrow-all"})
            self.log(f"Creating entity {self.name_manipulation_active} in HASS")
        ...
        if self.params.get('solar_heating_available'):
            ...
            if not self.entity_exists(self.name_solar_heating_active):
                ...
                self.set_state(entity_id=self.name_solar_heating_active, state=STATE_OFF, attributes={"friendly_name": name, "icon": "mdi:sun-thermometer"})
                self.log(f"Creating entity {self.name_solar_heating_active} in HASS")
            ...
            if not self.entity_exists(self.name_solar_heating_status):
                ...
                self.set_state(entity_id=self.name_solar_heating_status, state=STATE_OFF, attributes={"friendly_name": name, "icon": "mdi:sun-thermometer"})
                self.log(f"Creating entity {self.name_solar_heating_status} in HASS")
            self.input_booleans.append(self.name_solar_heating_status)
        else:
            if self.entity_exists(self.name_solar_heating_active):
                # When solar heating is not configured, but sensor exists, maybe it was configured in past.
                # But entity is a relict and should be deleted
                self.remove_entity(self.name_solar_heating_active)
                self.log(f"Removing entity {self.name_solar_heating_active} in HASS, configuration changed.")
            if self.entity_exists(self.name_solar_heating_status):
                # When solar heating is not configured, but sensor exists, maybe it was configured in past.
                # But entity is a relict and should be deleted
                self.remove_entity(self.name_solar_heating_status)
                self.log(f"Removing entity {self.name_solar_heating_status} in HASS, configuration changed.")
    ...
    def in_sun(self):
        ...
        sun_entry = -abs(self.params['facade']['facade_offset_entry'])  # Always negative
        sun_exit = abs(self.params['facade']['facade_offset_exit'])     # Always positive
    ...
    def calculate_angle(self):
        """
        Calculate optimal blind angle with improved handling of non-frontal sun angles
        """
        self.debug(f"Calculating angle")

        # Calculate relative azimuth (angle between sun and facade normal)
        relative_azimuth = self.azimuth - self.params['facade']['facade_angle']

        # Normalize relative azimuth to -180 to 180 degrees
        relative_azimuth = ((relative_azimuth + 180) % 360) - 180

        # If sun is behind facade, fully open blinds
        if abs(relative_azimuth) > 90:
            required_tilt = 0
        else:
            # Convert angles to radians
            sun_elevation_rad = math.radians(self.elevation)
            relative_azimuth_rad = math.radians(relative_azimuth)

            # Calculate effective spacing and width due to sun position
            # This accounts for the apparent change in slat geometry when sun is not frontal
            effective_spacing = self.params['blinds']['slat_distance'] / math.cos(relative_azimuth_rad)
            effective_width = self.params['blinds']['slat_width'] * math.cos(relative_azimuth_rad)

            # Calculate profile angle (apparent solar elevation in facade plane)
            profile_angle = math.atan(
                math.tan(sun_elevation_rad) / math.cos(relative_azimuth_rad)
            )

            # Calculate minimum required tilt to block direct sunlight
            required_tilt = math.degrees(math.atan(
                math.tan(profile_angle) * (effective_spacing / effective_width)
            ))

        # Convert angle (90-0 degrees) to percentage (0-100) with reversed mapping
        # 90° -> 0%
        # 0° -> 100%
        final_percentage = round(((90 - required_tilt) / 90) * 100)

        # Apply stepping
        final_percentage = round(final_percentage / self.params['blinds']['angle_step']) * self.params['blinds']['angle_step']

        # Add offset and ensure result stays within 0-100 range
        final_percentage = min(100, max(0, final_percentage + self.params['blinds']['angle_offset']))

        # Chheck min and max angle as floor and cap from config
        if final_percentage < self.params['move_contraints']['min_angle']:
            final_percentage = self.params['move_contraints']['min_angle']
        elif final_percentage > self.params['move_contraints']['max_angle']:
            final_percentage = self.params['move_contraints']['max_angle']
    ...
    def check_external_lock(self):
        if self.blinds_locked_external == STATE_ON:
            ...
            if self.blinds_locked_external_till is None:
                self.debug("Method check_external_lock not time found. Setting to off")
                self.set_state(entity_id=self.name_blinds_locked_external, state=STATE_OFF)
    ...
    def on_cover_change(self, entity, attribute, old, new, kwargs):
        ...
        if new is None or new['state'] in ["opening", "closing"]:
            ...
            # Check if new values are same as last calculated height respecting configured tolerance
            if old['attributes']['current_position'] != new['attributes']['current_position']:
                ...
                if self.current_height <= min((self.last_height + tolerance_height), 100) and self.current_height >= max((self.last_height - tolerance_height), 0):
                    ...
                    manual_change = True
            # Check if new values are same as last calculated angle respecting configured tolerance
            if old['attributes']['current_tilt_position'] != new['attributes']['current_tilt_position']:
                # Only check if position changed
                tolerance_angle = self.params['blinds']['angle_tolerance']
